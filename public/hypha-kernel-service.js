...
// Upgrade filesystem permission from read-only to read-write
async function upgradeFileSystemPermission() {
    if (!currentKernelId) {
        addOutput('stderr', 'No kernel selected');
        return;
    }

    const kernel = kernelManager.getKernel(currentKernelId);
    if (!kernel || !kernel.kernel.upgradeFileSystemPermission) {
        addOutput('stderr', '❌ Kernel does not support upgradeFileSystemPermission.');
        return;
    }

    try {
        addOutput('stdout', '🔧 Attempting to upgrade /mnt to write permission...');
        
        const result = await kernel.kernel.upgradeFileSystemPermission('/mnt');
        
        if (result.success) {
            addOutput('stdout', '✅ Successfully upgraded /mnt to WRITE permission!');
            addOutput('stdout', '⚡ Auto-sync is now enabled - changes will be saved automatically');
            addOutput('stdout', '💡 You can now write files:');
            addOutput('stdout', '   with open("/mnt/file.txt", "w") as f: f.write("Hello World!")');
        } else {
            addOutput('stderr', `❌ Failed to upgrade permission: ${result.error}`);
            addOutput('stdout', '💡 The browser may not allow write access to this directory.');
            addOutput('stdout', '   Try selecting a different directory or check browser permissions.');
        }
        
    } catch (error) {
        console.error('Upgrade permission error:', error);
        addOutput('stderr', `❌ Failed to upgrade permission: ${error.message}`);
    }
}

// Handle mount folder dropdown selection
function handleMountFolderSelect(event) {
    const permission = event.target.value;
    if (permission) {
        mountFolder(permission);
        // Reset dropdown to default state
        event.target.value = '';
    }
}

// Mount a native filesystem folder
async function mountFolder(permission = 'readwrite') {
    if (!currentKernelId) {
        addOutput('stderr', 'No kernel selected');
        return;
    }
    ...
    try {
        console.log('[UI] Showing directory picker...');
        const dirHandle = await window.showDirectoryPicker({
            mode: permission,
          });
        console.log('[UI] Directory selected:', dirHandle);
        ...
        if (!kernel || !kernel.kernel.mountFS) {
            addOutput('stderr', '❌ Kernel does not support mountFS.');
            return;
        }

        addOutput('stdout', '📁 Mounting directory... (check browser console for detailed logs)');
        console.log('[UI] Calling mountFS with dirHandle:', dirHandle);

        // Mount with the selected permission
        try {
            const nativefs = await kernel.kernel.mountFS('/mnt', dirHandle, permission);
            console.log(`[UI] mountFS returned successfully with ${permission}:`, nativefs);

            const permissionText = permission === 'readwrite' ? 'READ/WRITE' : 'READ-ONLY';
            addOutput('stdout', `✅ Successfully mounted directory to /mnt with ${permissionText} permission`);
            addOutput('stdout', '💡 You can read files in the mounted directory:');
            addOutput('stdout', '   import os; print(os.listdir("/mnt"))');
            
            if (permission === 'readwrite') {
                addOutput('stdout', '   with open("/mnt/file.txt", "w") as f: f.write("Hello!")');
                addOutput('stdout', '⚡ Auto-sync is enabled - changes will be saved automatically');
            } else {
                addOutput('stdout', '📖 Read-only mode: You cannot write files to this directory');
            }
            ...
            console.log(`[UI] Mount with ${permission} permission failed:`, mountError);
            
            addOutput('stderr', `❌ Failed to mount with ${permission} permission: ${mountError.message}`);
            
            if (permission === 'readwrite') {
                addOutput('stdout', '💡 Try selecting "Mount (Read-only)" from the dropdown if you only need to read files');
            }
            return;
        }
        ...
        if (error.name === 'AbortError') {
            addOutput('stdout', 'ℹ️ Directory selection cancelled');
        } else {
            addOutput('stderr', `❌ Failed to mount directory: ${error.message}`);
        }
    }
}
...
function updateButtonStates() {
    ...
    document.getElementById('mountFolderSelect').disabled = !selectedKernelId;
...
async function connectToHypha() {
    ...
    try {
        ...
        if (!window.hyphaWebsocketClient) {
            ...
            script.src = 'https://cdn.jsdelivr.net/npm/hypha-rpc@0.20.84/dist/hypha-rpc-websocket.min.js';
    ...
    try {
        ...
        const schemas = {
                    ...
                    properties: {
                        options: {
                            ...
                            properties: {
                                id: {
                                    type: "string",
                                    description: "Custom ID for the kernel (auto-generated UUID if not provided). Cannot contain colons."
                                },
                                mode: {
                                    type: "string",
                                    description: "Execution mode for the kernel. Worker mode (recommended) runs in a separate thread for better isolation. `worker` or `main_thread`",
                                    default: "worker"
                                },
                                namespace: {
                                    type: "string",
                                    description: "Optional namespace prefix to group related kernels. Applied as 'namespace:id'."
                                },
                                env: {
                                    type: "object",
                                    description: "Environment variables to set in the kernel (key-value pairs)",
                                },
                                lockFileURL: {
                                    type: "string",
                                    description: "URL to pyodide-lock.json file for faster loading with pre-installed packages",
                                },
                                autoSyncFs: {
                                    type: "boolean",
                                    description: "Automatically sync native filesystem after each code execution (requires mountFS)",
                                    default: false
                                },
                                inactivityTimeout: {
                                    type: "number",
                                    description: "Time in milliseconds after which an inactive kernel will be shut down (0 or undefined = no timeout)",
                                },
                                maxExecutionTime: {
                                    type: "number",
                                    description: "Maximum time in milliseconds a single execution can run before being considered stuck/dead (0 or undefined = no limit)",
                                }
                            }
                        }
                    },
            ...
            getMountedFileSystems: {
                ...
                description: "Get information about mounted filesystems for a specific kernel. Returns an array of mounted filesystem info including mount path, permission level (read/readwrite), and sync capability.",
                parameters: {
                    type: "object",
                    properties: {
                        kernelId: {
            ...
            syncFileSystem: {
                name: "syncFileSystem",
                description: "Manually sync a specific mounted filesystem for a kernel. This syncs changes from the Pyodide virtual filesystem back to the native mounted filesystem. Only works for filesystems mounted with readwrite permission.",
                parameters: {
                    type: "object",
                    properties: {
                        kernelId: {
                        ...
                        mountPath: {
                            type: "string",
                            description: "The path of the mounted filesystem to sync (e.g., '/mnt')"
                        }
                    },
                    required: ["kernelId", "mountPath"]
                }
            },
            upgradeFileSystemPermission: {
                name: "upgradeFileSystemPermission",
                description: "Upgrade a mounted filesystem from read-only to read-write permission. This requests write permission from the browser and remounts the filesystem if granted.",
                parameters: {
                    type: "object",
                    properties: {
                        kernelId: {
                        ...
                        mountPath: {
                            type: "string",
                            description: "The path of the mounted filesystem to upgrade (e.g., '/mnt')"
                        }
                    },
                    required: ["kernelId", "mountPath"]
                }
...
print(f"Successfully installed: {', '.join(${JSON.stringify(packages)})}")
            ...
            getMountedFileSystems: Object.assign(async ({ kernelId }, context = null) => {
                addOutput('result', `🌐 Remote call: getMountedFileSystems() - Getting mounted filesystems for kernel ${kernelId.substring(0, 8)}...`);

                try {
                    const filesystems = await kernelManager.getMountedFileSystems(kernelId);
                    addOutput('stdout', `Found ${filesystems.length} mounted filesystem(s)`);
                    return filesystems;
                } catch (error) {
                    addOutput('error', `❌ Failed to get mounted filesystems: ${error.message}`);
                    throw error;
                }
            }, { __schema__: schemas.getMountedFileSystems }),

            syncFileSystem: Object.assign(async ({ kernelId, mountPath }, context = null) => {
                addOutput('result', `🌐 Remote call: syncFileSystem() - Syncing filesystem at ${mountPath} for kernel ${kernelId.substring(0, 8)}...`);

                try {
                    const result = await kernelManager.syncFileSystem(kernelId, mountPath);

                    if (result.success) {
                        addOutput('result', `✓ Successfully synced filesystem at ${mountPath}`);
                    } else {
                        addOutput('error', `❌ Failed to sync filesystem: ${result.error}`);
                    }

                    return result;
                } catch (error) {
                    addOutput('error', `❌ Failed to sync filesystem: ${error.message}`);
                    throw error;
                }
            }, { __schema__: schemas.syncFileSystem }),

            upgradeFileSystemPermission: Object.assign(async ({ kernelId, mountPath }, context = null) => {
                addOutput('result', `🌐 Remote call: upgradeFileSystemPermission() - Upgrading permissions for ${mountPath} in kernel ${kernelId.substring(0, 8)}...`);

                try {
                    const result = await kernelManager.upgradeFileSystemPermission(kernelId, mountPath);

                    if (result.success) {
                        addOutput('result', `✓ Successfully upgraded filesystem permission at ${mountPath}`);
                    } else {
                        addOutput('error', `❌ Failed to upgrade permission: ${result.error}`);
                    }

                    return result;
                } catch (error) {
                    addOutput('error', `❌ Failed to upgrade filesystem permission: ${error.message}`);
                    throw error;
                }
            }, { __schema__: schemas.upgradeFileSystemPermission })
        });
        ...
        // Build the MCP server URL by replacing 'services/' with 'mcp/' and adding '/mcp' at the end
        const mcpServerUrl = `${serverUrl}/${workspace}/mcp/${actualServiceId}/mcp`;
        ...
        addOutput('stdout', `MCP Server URL: ${mcpServerUrl}`);
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        document.getElementById('mountFolderSelect').addEventListener('change', handleMountFolderSelect);
